...
def test_get_sqla_engine(mocker: MockerFixture) -> None:
    ...
    mocker.patch("superset.models.core._register_prequery_listener")
...
def test_get_sqla_engine_sets_thread_local_prequeries(
    app_context: None,
    mocker: MockerFixture,
) -> None:
    """
    Test that get_sqla_engine populates thread-local prequeries.

    Engines returned by get_sqla_engine must store prequeries in thread-local
    storage so the permanent connect listener can execute them on every new
    connection without dynamically adding/removing event listeners (which
    would cause deque mutation race conditions).
    """
    from superset.models.core import _PREQUERY_LOCAL
    ...
    with database.get_sqla_engine(catalog="my_catalog", schema="my_schema"):
        # While inside the context manager, prequeries should be in thread-local.
        store = getattr(_PREQUERY_LOCAL, "engines", {})
        assert store.get(id(mock_engine)) == ['SET search_path = "my_schema"']
    ...
    # After exiting the context manager, the entry should be cleaned up.
    store = getattr(_PREQUERY_LOCAL, "engines", {})
    assert id(mock_engine) not in store


def test_prequery_listener_cursor_closed_on_exception() -> None:
    """
    Test that the permanent prequery listener closes cursor even on error.

    Uses a real SQLite engine to verify the registered listener's behavior.
    """
    from sqlalchemy import create_engine

    from superset.models.core import (
        _PREQUERY_LISTENER_ENGINES,
        _PREQUERY_LOCAL,
        _register_prequery_listener,
    )

    engine = create_engine("sqlite:///:memory:")
    engine_oid = id(engine)
    _PREQUERY_LISTENER_ENGINES.discard(engine_oid)
    _register_prequery_listener(engine)

    # Set up a bad prequery that will raise an error.
    store: dict[int, list[str]] = {}
    store[engine_oid] = ["INVALID SQL THAT WILL FAIL"]
    _PREQUERY_LOCAL.engines = store

    # Attempting to create a connection should fail because the prequery is
    # invalid, but the important thing is it doesn't leave resources dangling.
    import pytest

    with pytest.raises(Exception, match="INVALID SQL"):  # noqa: B017
        engine.raw_connection()

    # Cleanup
    store.pop(engine_oid, None)
    _PREQUERY_LISTENER_ENGINES.discard(engine_oid)
...
def test_get_sqla_engine_no_prequeries_no_thread_local(
    ...
    """
    Test that get_sqla_engine does not populate thread-local prequeries when
    there are no prequeries to run.
    """
    from superset.models.core import _PREQUERY_LOCAL
...
def test_get_raw_connection_prequeries_via_thread_local(
    app_context: None,
    mocker: MockerFixture,
) -> None:
    """
    Test that get_raw_connection() makes prequeries available via thread-local
    storage for the permanent connect listener to execute.

    The permanent listener (registered once per engine via
    ``_register_prequery_listener``) reads from ``_PREQUERY_LOCAL`` to
    determine which prequeries to run, avoiding the deque mutation race.
    """
    from superset.models.core import _PREQUERY_LOCAL
    ...
    with database.get_raw_connection(schema="my_schema"):
        # Inside the context, thread-local should have the prequeries.
        store = getattr(_PREQUERY_LOCAL, "engines", {})
        assert store.get(id(mock_engine)) == [prequery]

    # After exiting, thread-local should be cleaned up.
    store = getattr(_PREQUERY_LOCAL, "engines", {})
    assert id(mock_engine) not in store


def test_prequery_thread_safety_no_deque_mutation() -> None:
    """
    Regression test for the 'deque mutated during iteration' race condition.

    Verifies that ``_register_prequery_listener`` registers a listener only
    once per engine (idempotent), and that prequeries are passed via
    thread-local storage rather than dynamic event listener add/remove.
    Concurrent threads using the same engine will each see their own
    prequeries without mutating the engine's event listener deque.
    """
    import threading
    ...
    # Register should be idempotent — calling twice must not duplicate listeners.
    _register_prequery_listener(engine)
    _register_prequery_listener(engine)
    assert engine_oid in _PREQUERY_LISTENER_ENGINES

    # Verify thread-local isolation: each thread sees only its own prequeries.
    results: dict[str, list[str] | None] = {}

    def thread_fn(name: str, prequeries: list[str]) -> None:
        store = getattr(_PREQUERY_LOCAL, "engines", None) or {}
        _PREQUERY_LOCAL.engines = store
        store[engine_oid] = prequeries
        # Simulate some work
        import time

        time.sleep(0.01)
        # Read back — should see only our own prequeries
        current_store = getattr(_PREQUERY_LOCAL, "engines", {})
        results[name] = current_store.get(engine_oid)
        store.pop(engine_oid, None)

    t1 = threading.Thread(target=thread_fn, args=("t1", ["SET schema=a"]))
    t2 = threading.Thread(target=thread_fn, args=("t2", ["SET schema=b"]))
    t1.start()
    t2.start()
    t1.join()
    t2.join()

    # Each thread should have seen its own prequeries.
    assert results["t1"] == ["SET schema=a"]
    assert results["t2"] == ["SET schema=b"]
